...
int bench_main(int argc, char** argv)
{
    ...
    benchmark_t *b = (benchmark_t *) malloc(sizeof(benchmark_t));
...
void simple_bench(const benchmark_t *b)
{
    ...
    switch (b->tag) {
        ...
        case SUMARRAY: {
            ...
            int64_t *ret_values = (int64_t *) malloc(NUM_ITERS * sizeof(int64_t));
            int64_t ret;
        ...
        case SORT: {
            ...
            int64_t *nums = (int64_t*)((*(b->sort_setup))(b->sort_total_elems));
            int64_t *sorted;

            // Run the benchmark.
            for (size_t i = 0; i < NUM_ITERS; i++) {
                clock_gettime(CLOCK_MONOTONIC_RAW, &begin_timed);
                sorted = ((int64_t *)(*(b->sort_run))(nums, b->sort_total_elems, b->sort_elt_size, b->sort_cmp));
                clock_gettime(CLOCK_MONOTONIC_RAW, &end_timed);
                itertime = difftimespecs(&begin_timed, &end_timed);
                printf("itertime: %lf\n", itertime);
                batchtime += itertime;
