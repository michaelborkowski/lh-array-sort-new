...
void *insertionsort_inplace(void *const pbase, size_t total_elems, size_t size);
...
template<typename T>
void *insertionsort_inplace(T *const pbase, size_t total_elems)
{
    for (int i = 0; i < total_elems; i++){
        T &elem = pbase[i];
        size_t s = i;
        while(s > 0 && pbase[s] > elem){
            pbase[s] = pbase[s-1];
            s = s - 1;
        }
        pbase[s] = elem;
