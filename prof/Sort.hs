...
merge_parmon !src1 !src2 !dst =
    ...
    then pure $ merge src1 src2 dst
             ...
             dst' = dst
             in if n1 == 0
                then do !(src2'1, dst'') <- C.copy2_parmon src2' 0 dst' 0 n2
                        pure (A.append src1' src2'1, dst'')
                else if n2 == 0
                     then do !(src1'1, dst'') <- C.copy2_parmon src1' 0 dst' 0 n1
...
msort_parpseq src =
  let (len, src') = A.size2 src in
      if len == 0 then src'
               ...
               src''' = A.make len x0
               (_,src'''') = C.copy2_parpseq src'' 0 src''' 0 len
...
merge_parpseq !src1 !src2 !dst =
    ...
    then merge src1 src2 dst
             ...
             dst' = dst
             in if n1 == 0
                then let !(src2'1, dst'') = C.copy2_parpseq src2' 0 dst' 0 n2
                     in (A.append src1' src2'1, dst'')
                else if n2 == 0
                     then let !(src1'1, dst'') = C.copy2_parpseq src1' 0 dst' 0 n1
