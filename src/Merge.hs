...
merge [] ys _ _ = ys
merge xs ys 0 0 = (make ((size xs)+(size ys)) (A.get xs 0))
merge xs ys 0 m = let zs = merge xs ys 0 (m-1) in (set zs (m-1) (A.get ys (m-1)))
merge xs ys n 0 = let zs = merge xs ys (n-1) 0 in (set zs (n-1) (A.get xs (n-1)))
