...
-- module Array  where -- (Array, make, get, set, size, lma_gs, lma_gns)
...
--       ( lst   : [a], 
--         left  : {v:Nat | v >= 0},
--         right : {v:Nat | v >= le && v <= lengthZ lst}
--       ) 
--   @-}

-- type Array a = ([a], Int, Int)
...
-- make :: Int -> a -> Array a
-- make 0 x = ([],0,0)
-- make n x = let (lst, left, right) = (make (n-1) x)
--            in ((x:lst), left, right+1)
