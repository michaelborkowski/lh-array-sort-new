...
-- merge' [] ys _ _ = ys
merge' xs ys zs 0 0 = zs -- (make ((size xs)+(size ys)) (get xs 0))
merge' xs ys zs 0 m = let zs' = merge' xs ys zs 0 (m-1) in (set zs' (m-1) (get ys (m-1)))
merge' xs ys zs n 0 = let zs' = merge' xs ys zs (n-1) 0 in (set zs' (n-1) (get xs (n-1)))
