...
pfsort' :: (Show a, HasPrimOrd a) => Int -> a -> A.Array a -. A.Array a
pfsort' cutoff anyVal src =
  let !(Ur len, src') = A.size2 src  -- below expression is always in the interval [28, 708] (interval changed from meeting doc).
      !(src'', _tmp) = msortInplace cutoff src' (A.makeArray len anyVal) in
  case A.free _tmp of !() -> src''
{-# INLINE pfsort' #-}

{-@ pfsort :: n:Int -> { xs:(A.Array a) | left xs == 0 && right xs == size xs }
                    -> { ys:_ | toBag xs == toBag ys && isSorted' ys &&
                                A.size xs == A.size ys && token xs == token ys  } @-}
pfsort :: (Show a, HasPrimOrd a) => Int -> A.Array a -. A.Array a
pfsort cutoff src =
  let !(Ur len, src') = A.size2 src in
      if len == 0 then src'
      else let !(Ur x0, src'') = A.get2 0 src' in pfsort' cutoff x0 src''
{-# INLINABLE pfsort #-}
