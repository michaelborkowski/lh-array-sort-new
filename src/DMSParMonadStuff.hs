...
import qualified Control.Monad.Par as P (Par, runPar, spawnP, spawn_, get, fork, put, put_, new)
#else
import           Array.List as A
#endif

#define KILO     1024
#define	SEQSIZE  2*KILO
...
{-@ msortSwap :: xs:Array a 
      ...
      -> (Array a, Array a)<{\zs ts -> toBag xs == toBag ts && isSorted' ts &&
       ...
       / [A.size xs] @-}
msortSwap :: (Show a, Ord a, NFData a) => A.Array a -> A.Array a -> P.Par (A.Array a, A.Array a)
msortSwap src tmp = do
  let (len, src') = A.size2 src 
  if len <= 1
  then do
          (src'', tmp'') <- copy src' tmp 0 0 
          return (src'', tmp'')
  else do
    ...
    return (src'', tmp4)  ? lem_toBag_splitMid src -- tmp4 holds the sorted data
...
{-@ msortInplace :: xs:Array a 
      ...
      -> (Array a, Array a)<{\zs ts -> toBag xs == toBag zs && isSorted' zs &&
                                       ...
                                       left ts == left ys && right ts == right ys }> 
       / [A.size xs] @-}
msortInplace :: (Show a, Ord a, NFData a) => A.Array a -> A.Array a -> P.Par (A.Array a, A.Array a)
msortInplace src tmp = do
  let (len, src') = A.size2 src 
  if len <= SEQSIZE
  then do
          (src'', tmp'') <- Seq.msortInplace src' tmp
          return  (src'', tmp'')
  else do
    let (src1, src2)   = splitMid src'
        (tmp1, tmp2)   = splitMid tmp
    ...
    return (src4', tmp'')  ? lem_toBag_splitMid src -- src4' holds the sorted data
                           ? lem_toBag_splitMid tmp
...
msort' src anyVal = P.runPar $ do
  let (len, src') = A.size2 src
  (src'', _tmp) <- msortInplace src' (A.make len anyVal) 
  {-_tmp `seq`-} 
  return src''
