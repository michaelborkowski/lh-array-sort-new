...
merge_par_func src1 src2 dst {-!((src1, src2), dst)-} =
    if A.size dst < goto_seqmerge -- unfold this guard?
...
{-@ lem_merge_par_func_inv_left  :: { xs1:(Array a) | isSorted' xs1 }
      ...
      -> { pf:_   | size zs == 0 || piv <= get (merge_par_func xs1 xs2 zs) 0 } @-}
lem_merge_par_func_inv_left :: HasPrimOrd a => A.Array a -> A.Array a -> A.Array a -> a -> Proof
lem_merge_par_func_inv_left src1 src2 dst piv = undefined
