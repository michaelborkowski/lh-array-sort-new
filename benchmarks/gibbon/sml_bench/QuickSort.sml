...
and shuffleBtw xs i j = 
  let val tup_42 = (get2 xs (j - 1)) in 
  let val piv = (case tup_42 of (x__, _) => x__) in 
  let val xs1 = (case tup_42 of (_, x__) => x__) in 
  let val goShuffle = (fn zs => fn jl => fn jr => 
  (if (jl > jr) then (zs ,  jl) 
   else 
  let val tup_38 = (get2 zs jl) in 
  let val vl = (case tup_38 of (x__, _) => x__) in 
  let val zs' = (case tup_38 of (_, x__) => x__) in 
  (if (vl <= piv) then (goShuffle zs' (jl + 1) jr) 
   else 
  let val tup_34 = (get2 zs' jr) in 
  let val vr = (case tup_34 of (x__, _) => x__) in 
  let val zs'' = (case tup_34 of (_, x__) => x__) in 
  (if (vr > piv) then (goShuffle zs'' jl (jr - 1)) 
   else 
  let val zs''' = (swap zs'' jl jr) in (goShuffle zs''' jl (jr - 1)) end) end end end) end end end)) in 
  let val tup_25 = (goShuffle xs1 i (j - 2)) in 
  let val xs' = (case tup_25 of (x__, _) => x__) in 
  let val ip = (case tup_25 of (_, x__) => x__) in 
  let val xs'' = 
...
and quickSortBtw xs i j = 
  ...
  let val tup_17 = (shuffleBtw xs i j) in 
  let val xs' = (case tup_17 of (x__, _) => x__) in 
  let val i_piv = (case tup_17 of (_, x__) => x__) in 
  let val xs'' = (quickSortBtw xs' i i_piv) in 
  let val xs''' = (quickSortBtw xs'' (i_piv + 1) j) in xs''' end end end end end)
and quickSort xs cmp = 
  let val tup_8 = (size2 xs) in 
  let val len = (case tup_8 of (x__, _) => x__) in 
  let val xs1 = (case tup_8 of (_, x__) => x__) in 
  let val tup_4 = (get2 xs1 0) in 
  let val hd = (case tup_4 of (x__, _) => x__) in 
  let val xs2 = (case tup_4 of (_, x__) => x__) in 
  let val cpy = (alloc len hd (fn tmp => (copy xs2 0 tmp 0 len))) in (quickSortBtw cpy 0 len) end end end end end end end
